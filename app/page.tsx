...
export default function SportsSpriteDashboard() {
  ...
  const handleGenerate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const currentGen: SpriteGeneration = {
        id: prediction.id,
        sport,
        style,
        pose,
      ...
      const checkStatus = setInterval(async () => {
        try {
          const statusRes = await fetch(`/api/generate?id=${prediction.id}`);
          const statusData = await statusRes.json();

          if (statusData.status === 'succeeded') {
            clearInterval(checkStatus);
            
            // Handle output string or array from Replicate safely
            const dynamicUrl = Array.isArray(statusData.output) 
              ? statusData.output[0] 
              : statusData.output;
            ...
            clearInterval(checkStatus);
            setLoading(false);
            alert("AI engine encountered an error drawing this sprite configuration.");
          }
        } catch (err) {
          clearInterval(checkStatus);
          setLoading(false);
      ...
      console.error(err);
      setLoading(false);
      alert("Something went wrong. Please check your network or Vercel Environment variables.");
    }
  };
